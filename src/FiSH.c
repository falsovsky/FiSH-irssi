...
static void DH1080_received(SERVER_REC * server, char *msg, char *nick,
			    char *address, char *target);
static void DH1024_received(SERVER_REC * server, char *msg, char *nick,
			    char *address, char *target);
...
size_t irssi_recode_text(char *text, size_t n,
			 const SERVER_REC * server,
			 const char *target, size_t max_n)
{
 ...
 */
int FiSH_encrypt(const SERVER_REC * serverRec,
		 const char *msgPtr,
		 const char *target, char *bf_dest, size_t n)
{
	...
	int ret = fish2_encrypt(fish2_ctx,
 ...
 */
int FiSH_decrypt(const SERVER_REC * serverRec,
		 char *msg_ptr, char *msg_bak, const char *target)
{
	char bf_dest[1000];
	...
	if (fish2_decrypt(fish2_ctx,
			  server_tag(serverRec),
			  target, msg_ptr, bf_dest, 1000) < 0) {
		return 0;
	}
...
void format_msg(SERVER_REC * server, char *msg, char *target, char *orig_target)
{
	...
	if (fish2_mark_encryption(fish2_ctx,
				  ...
				  target,
				  msg,
				  0, formattedMsg, sizeof(formattedMsg)) < 0) {
		return;
	}
...
{				// New command: /me+ <action message>
	...
	if (FiSH_encrypt(server, (char *)data, target, bf_dest, sizeof(bf_dest))
	    == 0) {
		printtext(server, target, MSGLEVEL_CRAP,
...
void cmd_setinipw(const char *iniPW, SERVER_REC * server, WI_ITEM_REC * item)
{
	...
	if (iniPW != NULL) {
		...
		if (strfcpy(clean_key, iniPW, sizeof(clean_key)) == NULL)
			return;
...
void cmd_setkey(const char *data, SERVER_REC * server, WI_ITEM_REC * item)
{
	...
	const char *target_window =
	    (item ? window_item_get_target(item) : NULL);

	printtext(server, target_window, MSGLEVEL_CRAP,
		  "\002FiSH\002: Setting key");
...
void cmd_keyx(const char *target, SERVER_REC * server, WI_ITEM_REC * item)
{
	...
	irc_send_cmdv((IRC_SERVER_REC *) server,
		      "NOTICE %s :%s %s",
		      target, "DH1080_INIT", keyx_public_key(keyx_ctx));
			    ...
			    char *address, char *target)
{
	...
	if (strncmp(msg, "DH1080_INIT ", 12) == 0) {
		...
		irc_send_cmdv((IRC_SERVER_REC *) server,
			      "NOTICE %s :%s %s",
			      nick, "DH1080_FINISH", keyx_public_key(keyx_ctx));
...
void query_nick_changed(QUERY_REC * query, char *orignick)
{
	...
	if (fish2_get_key
	    (fish2_ctx, server_tag(query->server), orignick, theKey) < 0)
		return;		// see if there is a key for the old nick

	if (fish2_set_key
	    (fish2_ctx, server_tag(query->server), query->name, theKey) < 0) {
		printtext(NULL, NULL, MSGLEVEL_CRAP,
			  "\002FiSH ERROR:\002 Unable to write to blow.ini, probably out of space or permission denied.");
	}
...
void fish_init(void)
{
	...
	if (keyx_init(&keyx_ctx) < 0)
		return;

	if (fish2_init(&fish2_ctx, iniPath) < 0)
		return;
